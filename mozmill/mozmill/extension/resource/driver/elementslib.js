...
var _forAnonChildren = function (_document, element, name, value) {
  var results = [];
  var nodes = [];
  for (var e of _document.getAnoymousNodes(element)) {
    if (e) {
      nodes.push(e);
    }
  }
...
var _byAnonAttrib = function (_document, parent, attributes) {
  ...
  var nodes = [];
  for (var n of _document.getAnonymousNodes(parent)) {
    if (n.getAttribute) {
      nodes.push(n);
    }
  }
  ...
  if (results.length == 0) {
    var ns = [];
    for (var n of parent.childNodes) {
...
function Lookup(_document, expression) {
  ...
  var expSplit = [];
  for (var e of smartSplit(expression)) {
    if (e != '') {
      expSplit.push(e);
    }
  }
